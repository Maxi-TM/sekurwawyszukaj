...
function renderScoreHud() {
	if (isCasualGame()) {
		...
		scoreNode.innerText = `WYNIK: ${state.game.score}`;
		scoreNode.style.display = 'block';
		cubeCountNode.style.opacity = 0.65 ;
	}
	cubeCountNode.innerText = `ZNISZCZONE KOSTKI: ${state.game.cubeCount}`;
}
...
function renderMenus() {
	...
	switch (state.menus.active) {
		...
		case MENU_SCORE:
			...
			if (isNewHighScore()) {
				highScoreLblNode.textContent = 'Nowy Najwyższy Wynik!';
			} else {
				highScoreLblNode.textContent = `Najwyższy Wynik: ${formatNumber(getHighScore())}`;
			}
